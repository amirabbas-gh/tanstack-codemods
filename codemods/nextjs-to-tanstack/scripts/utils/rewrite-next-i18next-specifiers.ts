...
export function stripNextI18nextServerSideTranslationsImport(
  source: string,
): string {
 ...
 */
export function rewriteNextI18nextMainAndDynamicImports(
  source: string,
): string {
  let s = source;
  s = s.replace(/from\s+(["'])next-i18next\1/g, "from $1react-i18next$1");
  s = s.replace(
    /import\s*\(\s*(["'])next-i18next\1\s*\)/g,
    "import($1react-i18next$1)",
  );
  return s;
}

/** Used by R10a app-route pipeline and the cross-cutting i18n import step. */
export function applyNextI18nextToReactI18nextModuleRewrites(
  source: string,
): string {
