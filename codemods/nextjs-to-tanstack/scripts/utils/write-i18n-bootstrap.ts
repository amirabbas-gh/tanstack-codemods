/**
 * Emits `src/i18n.ts` (or root `i18n.ts`) so `react-i18next` has a real instance after
 * `next-i18next` / `serverSideTranslations` are gone. Loads `public/locales/<lng>/common.json`
 * on the server via `node:fs` and in the browser via fetch (no extra npm deps).
 */
...
/** True if a file exists and is readable (JSSG fs has limited exports). */
...
/** Writes i18n bootstrap once; does not overwrite if the file already exists. */
