...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const { name, phone, company, message, meet_type, formName, formType, resume } = req.body;
    ...
    if (!recipientEmail) {
        return res.status(400).json({ error: 'نوع فرم نامعتبر است.' });
    }
  ...
  `,
        ...
        return res.status(200).json({ message: 'پیغام شما با موفقیت ارسال شد.' });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ error: 'ارسال ایمیل با مشکل مواجه شد.' });
    }
}
